...
segy_file* segy_open( const char* path, const char* mode ) {
    ...
    bool rw = strstr( binary_mode, "+" ) || strstr( binary_mode, "w" );
...
int segy_mmap( segy_datasource* ds ) {
    ...
    /* save properties that are inherited by memdatasource */
    bool writable =  ds->writable;
    bool lsb = ds->lsb;
    int elemsize = ds->elemsize;
...
int segy_writesubtr( segy_datasource* ds,
    ...
    if( err != 0 ) { free( tracebuf ); return SEGY_DS_READ_ERROR; }
