...
def test_traceheaders(small):
    with segyio.open(small, "r+") as f:
        assert isinstance(f.traceheader[0], segyio.trace.TraceHeaderSequence)
        ...
        f.traceheader[2][0:1] = f.traceheader[2][0:1]
        assert list(f.traceheader[2][0:1]) == list(f.traceheader[2][0:1])


def test_traces_traceheaders(small):
    with segyio.open(small, "r+") as f:
        assert isinstance(
            f.traceheader, segyio.trace.TracesTraceHeaderSequence)
...
def test_tracefields():
    with segyio.open(testdata / 'trace-header-extensions.sgy') as f:
        ...
        for x in f.tracefield:
            assert isinstance(x, segyio.trace.TraceFieldLayoutSequence)

        assert len(f.tracefield[1]) == 2
        for x in f.tracefield[1]:
            assert isinstance(x, segyio.trace.TraceFieldEntry)
