...
class Stanza(Sequence):
    ...
    Reading stanzas is done with []. Keys are stanza indecies, values are raw
...
class TraceHeaderLayoutSequence(Sequence):
    """
    Sequence of trace header layouts in a SEG-Y file.
    ...
    def __getitem__(self, key):
        ...
        Returns
        -------
        TraceFieldLayoutSequence or generator of TraceFieldLayoutSequence
        ...
        try:
            ...
            return TraceFieldLayoutSequence(self.segyfile, traceheader_index, layout)
        except TypeError:
            indices = key.indices(len(self))
            ...
            def gen():
                for traceheader_index in range(*indices):
                    name = self._layout_names[traceheader_index]
                    layout = self._layouts[name]
                    yield TraceFieldLayoutSequence(self.segyfile, traceheader_index, layout)
            return gen()
    ...
    def __getattr__(self, name):
        ...
        if name in self._layouts:
            ...
            return TraceFieldLayoutSequence(self.segyfile, traceheader_index, self._layouts[name])
    ...
    def __repr__(self):
        return f"TraceHeaderLayoutSequence({self._layout_names})"


class TraceFieldLayoutSequence(Sequence):
    """
    Sequence of fields in a trace header.
    ...
    def __getitem__(self, key):
        ...
        Returns
        -------
        TraceFieldEntry or generator of TraceFieldEntry
        ...
        try:
            ...
            return TraceFieldEntry(self.segyfile, entry, self.traceheader_index)
            ...
            def gen():
                for entry_index in range(*indices):
                    entry = self._layout.entries[entry_index]
                    yield TraceFieldEntry(self.segyfile, entry, self.traceheader_index)
            return gen()
    ...
    def __repr__(self):
        return f"TraceFieldLayoutSequence({self.names()})"


class TraceFieldEntry():
    """
